...
from test.pageObject.pageObjectModel.vwo.DashboardPage import DashboardPage
...
@allure.title("VWO Login Test_Negative")
...
def test_vwo_login_positive(setup):
    driver = setup
    login_page = LoginPage(driver=driver)
    login_page.login_to_vwo(usr=os.getenv("USERNAME4"), pwd=os.getenv("PASSWORD4"))
